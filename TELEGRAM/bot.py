import telebot #Библиотека для подключения телеграм бота
from telebot import types #Для создания кнопок. 
from openpyxl import load_workbook #Это библиотека Python для чтения/записи форматов Office Open XML. 
import string #Библиотека для общих операций со строками.
...
while True:
    ...
    def fizvoz(message):
        ...
        item2 = types.KeyboardButton('200')   #Создание кнопок в отделении "Прикладная информатика"
    ...
    def pk(message):
        ...
        item2 = types.KeyboardButton('203')   #Создание кнопок в отделении "Прикладная информатика"
    ...
    def elementary_grades(message):
        ...
        item4 = types.KeyboardButton('201 Б')    #Создание кнопок в отделении "Преподавания в начальных классах"
    ...
    def preschool_education(message):
        ...
        item2 = types.KeyboardButton('202')   #Создание кнопок в отделении "Дошкольное образование"
    ...
    def close(message):
        ...
        item2 = types.KeyboardButton('👼 Дошкольное образование')        #Создание кнопок для отделений
    ...
    def start(message):
        ...
        item4 = types.KeyboardButton('💻 Прикладная информатика')
        markup.add(item1, item2, item3, item4)
        bot.send_message(message.chat.id, 'Привет, {0.first_name}!'.format(message.from_user), reply_markup=markup) #При
        # запуске бота появляется приветствие с вашим именем и отображением кнопок отделений
    ...
    bot.polling(none_stop=True) #не прекращать опрос при получении ошибки от серверов Telegram